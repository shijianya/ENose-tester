...
# 设置中文字体
matplotlib.rcParams['font.sans-serif'] = ['Microsoft YaHei']  # 替换为系统支持的中文字体
...
def open_resistor_dialog():
    ...
    for i, (key, value) in enumerate(resistor_values.items()):
        ...
        tk.Button(resistor_window, text="输入", font=("Microsoft YaHei", 15), command=make_set_function(key, var)).grid(row=i, column=2, padx=5)
...
def update_resistor_value(key, var):
    try:
        ...
        save_config()  # 直接调用保存
        print(f"{key} 更新为 {new_value}")
    except ValueError:
        messagebox.showerror("错误", f"{key} 的输入无效，请输入数字")

def toggle():
    global running, ser, all_data, x_data, channel_count
    if running:
        stop_acquisition()
        start_btn.config(text="开始")           
    else:
        all_data.clear()
        channel_count = int(channel_cb.get()) + 1
        source = source_var.get()
        running = True
        start_btn.config(text="停止")
        init_plot()
        ...
        if source == "serial":
            ...
            try:
                ...
                messagebox.showerror("BLE 错误", str(e))
                running = False
                start_btn.config(text="开始")
...
def update_plot(values):
    ...
    if running:
        try:            
            ...
            # 更新实时数据显示
            pressure_label.config(text=f"气压: {pressure:.1f}kPa")
            humidity_label.config(text=f"湿度: {humidity:.1f}%")                
            temperature_label.config(text=f"温度: {temperature:.1f}°C")
            Voltage_C.config(text=f"总电压: {voltage0:.3f}V")
            Time_value.config(text=f"时间: {index/2:.1f}S")
...
def save_data():
    ...
    if file_path:
        ...
        messagebox.showinfo("提示", f"数据已保存到 {file_path}")
...
def connect_selected_ble():
    ...
    if not selection or '(' not in selection:
        messagebox.showerror("错误", "请选择有效的 BLE 设备")
        return
...
# ===== 选择数据源类型 =====
ttk.Label(config_frame, text="数据源:").grid(row=0, column=0, padx=5, pady=5, sticky="w")
...
# ===== 串口设置 =====
# 串口设置
ttk.Label(serial_frame, text="串口:").grid(row=0, column=0, padx=5, pady=5)
...
scan_button = ttk.Button(ble_frame, text="扫描设备")
...
#通道数
ttk.Label(config_frame, text="通道数:").grid(row=0, column=2, padx=5, pady=5, sticky="w")
...
# 按钮部分
ttk.Button(config_frame, text="设置参考电阻", command=open_resistor_dialog).grid(row=0, column=4, padx=5, pady=5, sticky="w")
start_btn = ttk.Button(config_frame, text="开始", command=toggle)
start_btn.grid(row=0, column=5, padx=5, pady=5, sticky="w")
ttk.Button(config_frame, text="保存数据", command=save_data).grid(row=0, column=6, padx=5, pady=5, sticky="w")

# 实时数据显示
data_frame = ttk.LabelFrame(root, text="实时数据")
data_frame.pack(fill="x", padx=10, pady=5)

pressure_label = tk.Label(data_frame, text="气压: --kPa", font=("Microsoft YaHei", 14))
pressure_label.grid(row=0, column=0, padx=10, pady=10)

humidity_label = tk.Label(data_frame, text="湿度: --%", font=("Microsoft YaHei", 14))
humidity_label.grid(row=0, column=1, padx=10, pady=10)

temperature_label = tk.Label(data_frame, text="温度: --°C", font=("Microsoft YaHei", 14))
temperature_label.grid(row=0, column=2, padx=10, pady=10)

Voltage_C = tk.Label(data_frame, text="总电压：--V", font=("Microsoft YaHei", 14))
Voltage_C.grid(row=0, column=5, padx=10, pady=10)

Time_value = tk.Label(data_frame, text="时间：--s", font=("Microsoft YaHei", 14))
Time_value.grid(row=0, column=9, padx=10, pady=10)

#图形区
fig, axes, lines = plt.figure(figsize=(8, 8)), [], []
